...
export default class ParkingLot {
    ...
    public park(car: ICar): number | void {
        ...
        this._carsColourSlot[car.colour] = [
            ...(this._carsColourSlot[car.colour] || []),
            availableSlot,
        ]
    ...
    public leave(slot: number): boolean {
        ...
        this._carsColourSlot[carToLeve.colour] = this._carsColourSlot[
            carToLeve.colour
        ].filter(x => x !== slot)
        ...
        return (
            ...
            delete this._carsRegSlot[carToLeve.registrationNumber]
        )
    }
    public getAllParkedCars() {
        return Object.values(this._carsRegSlot).map(slot => ({
            'Slot No.': slot,
            'Registration No': this._slots[slot]['registrationNumber'],
            Colour: this._slots[slot]['colour'],
        }))
    }
    public getCarSlotForRegistrationNumber(regNo: string): number | undefined {
        return this._carsRegSlot[regNo]
    }
    public getCarsSlotBasedOnColour(colour: string): number[] {
        return this._carsColourSlot[colour] || []
    }
    public getRegNosBasedOnColour(colour: string): string[] {
        return this._carsColourSlot[colour]
            ? this._carsColourSlot[colour].map(
                  slot => this._slots[slot]['registrationNumber']
              )
            : []
    }
}
